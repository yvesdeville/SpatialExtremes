...
\begin{tabular}{lll}
  ...
  c_1 \leq 1$, $c_2 >0$, $\nu > 0$\\
  \textbf{Powered Exponential}\index{covariance function!powered
    exponential} & $\rho(h) = c_1 \exp\left[- \left(\frac{h}{c_2}
...
Bessel function of the third kind with order
$\nu$. Figure~\ref{fig:covariances} plots the correlation functions
...
corresponds to the variance of the pairwise score
equations\index{score equation} $\ell_p(\psi;\mathbf{Y}) =
...
function. The second estimator is only accessible if independent
