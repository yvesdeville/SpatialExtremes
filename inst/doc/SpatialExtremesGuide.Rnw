...
data <- rmaxstab(40, locations, "whitmat", sill = 1, range = 1, smooth = 2)

##Compute the lambda-madogram
lmadogram(data, locations, n.bins = 25, border = "grey", box = FALSE)
@ 
  \end{center}
\end{titlepage}
...
par(mfrow=c(1,2))
image(x, x, data0[,,1], col = terrain.colors(40))
image(x, x, data1[,,1], col = terrain.colors(40))
@ 
...
function $\rho(h)$, scaled so that $\mathbb{E}[\max\{0, Y_i(x)\}] =
1$. With these new assumptions, it can be shown that the bivariate CDF
...
\begin{tabular}{lll}
  ...
  & $\rho(h) = c_1 \frac{2^{1-\nu}}{\Gamma(\nu)} \left(\frac{h}{c_2}
  \right)^{\nu} K_{\nu}\left(\frac{h}{c_2} \right)$, & $0 \leq c_1
  \leq 1$, $c_2>0$, $\nu > 0$\\
  \textbf{Cauchy}\index{covariance function!cauchy} & $\rho(h) = c_1
  \left[1 + \left(\frac{h}{c_2} \right)^2 \right]^{-\nu}$, & $0 \leq
  c_1 \leq 1$, $c_2 >0$, $\nu > 0$\\
  \textbf{Powered Exponential}\index{covariance function!powered exponential} & $\rho(h) = c_1 \exp\left[- \left(\frac{h}{c_2}
    \right)^{\nu} \right]$, & $0 \leq c_1 \leq 1$, $c_2 > 0$, $0 < \nu
  \leq 2$\\
  \textbf{Bessel}\index{covariance function!bessel} & $\rho(h) = c_1
...
where $c_1$, $c_2$ and $\nu$ are the sill, 
\index{covariance function!sill} the range
\index{covariance function!range} and the smooth
\index{covariance function!smooth} parameters of the correlation
function, $\Gamma$ is the gamma function and $J_{\nu}$ and $K_{\nu}$
are the Bessel and the modified Bessel function of the third kind with
order $\nu$ and $d$ is the dimension of the random
fields. Figure~\ref{fig:covariances} plots the correlation functions
for the parametric families introduced above. The left panel was
generated with the following lines
<<eval=FALSE>>=
covariance(sill = 1, range = 1, smooth = 4, cov.mod = "whitmat",
           xlim = c(0,9), ylim = c(0, 1))
covariance(sill = 1, range = 1, smooth = 2, cov.mod = "whitmat",
...
data3 <- rgp(1, coord, cov.mod = "whitmat", sill = 1, range = 1, smooth = 1,
             grid = TRUE, nlines = 625)

par(mfrow=c(1,3))
image(x, x, data1[,,1], col = terrain.colors(30))
image(x, x, data2[,,1], col = terrain.colors(30))
image(x, x, data3[,,1], col = terrain.colors(30))
@ 
...
par(mfrow = c(1,2))
image(x, x, M0[,,1]^0.25, col = terrain.colors(30))
image(x, x, M1[,,1]^0.25, col = terrain.colors(30))
@ 
...
\caption{Two simulations of the Smith model with different covariance
  ...
  $\sigma_{12} = 0$. Right panel: $\sigma_{11} = \sigma_{22} = 9/8$,
  $\sigma_{12} = 3/4$. The observations are scaled for viewing
  purposes.}
...
data1 <- rmaxstab(100, coord, "whitmat", sill = 0.8, range = 1, smooth = 1)
@ 
...
likelihood ratio test is around $0.80$ which advocates the use of
