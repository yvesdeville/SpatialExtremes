...
void wsmithstderr(double *data, double *distVec, int *nSite,
  ...
  for (i=0;i<(*nSite-1);i++){
	...
	for (k=0;k<*nObs;k++){
	  ...
	  jacCommonSigma = weights[currentPair] * 
	    (dAa + (dBa * C + B * dCa + dDa) / (B*C + D));
	...
	if (weights[currentPair] != 0){
	  for (k=0;k<*nObs;k++){
		...
		scales[j] * locdsgnmat[j + *nSite * l];
	      ...
	      hess[((3 + l) * *nObs + k) * nPairs + currentPair] = weights[currentPair] *
		((dAz1 * dz1loc + dAz2 * dz2loc) + ((dBz1 * dz1loc + dBz2 * dz2loc) * C + B * 
		...
		scaledsgnmat[j + *nSite * l];
	      ...
	      hess[((3 + *nloccoeff + l) * *nObs + k) * nPairs + currentPair] = weights[currentPair] *
		((dAz1 * dz1scale + dAz2 * dz2scale) + ((dBz1 * dz1scale + dBz2 * dz2scale) * C + B * 
		...
		weights[currentPair] * ((dAz1 * dz1shape + dAz2 * dz2shape) +
	...
	if (weights[currentPair] != 0){	 
	  for (k=0;k<*nObs;k++){
		...
		scales[j] * locdsgnmat[j + *nSite * l];	    
	      ...
	      hess[((6 + l) * *nObs + k) * nPairs + currentPair] = weights[currentPair] *
		((dAz1 * dz1loc + dAz2 * dz2loc) + ((dBz1 * dz1loc + dBz2 * dz2loc) * C + B * 
						    (dCz1 * dz1loc + dCz2 * dz2loc) +
						    (dDz1 * dz1loc + dDz2 * dz2loc)) *
		 iBCplusD + dE);
		...
		scaledsgnmat[j + *nSite * l];
	      ...
	      hess[((6 + *nloccoeff + l) * *nObs + k) * nPairs + currentPair] = weights[currentPair] *
		((dAz1 * dz1scale + dAz2 * dz2scale) + ((dBz1 * dz1scale + dBz2 * dz2scale) * C + B * 
							(dCz1 * dz1scale + dCz2 * dz2scale) +
							(dDz1 * dz1scale + dDz2 * dz2scale)) *
		 iBCplusD + dE);
	...
	if (weights[currentPair] != 0){
	  for (k=0;k<*nObs;k++){
		...
		scales[j] * locdsgnmat[j + *nSite * l];
	      ...
	      hess[((nCorPar + l) * *nObs + k) * nPairs + currentPair] = weights[currentPair] *
		...
		weights[currentPair] * ((dAz1 * dz1scale + dAz2 * dz2scale) +
	...
	if (weights[currentPair] != 0){
	  for (k=0;k<*nObs;k++){
		...
		scales[j] * locdsgnmat[j + *nSite * l];
	      ...
	      hess[((2 + l) * *nObs + k) * nPairs + currentPair] = weights[currentPair] *
		((dAz1 * dz1loc + dAz2 * dz2loc) + ((dBz1 * dz1loc + dBz2 * dz2loc) * C + B * 
						    (dCz1 * dz1loc + dCz2 * dz2loc) +
						    (dDz1 * dz1loc + dDz2 * dz2loc)) *
		 iBCplusD + dE);
