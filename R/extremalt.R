...
extremaltfull <- function(data, coord, start, cov.mod = "whitmat", ...,
    ...
    body(nplk) <- parse(text = paste("-.C(C_extremaltfull, as.integer(cov.mod.num), as.double(data), as.double(dist), as.integer(n.site), as.integer(n.obs), as.integer(dist.dim), as.integer(weighted), as.double(weights),",
                        ...
                        "as.double(nugget), as.double(range), as.double(smooth), as.double(smooth2), as.double(DoF), fit.marge, dns = double(1), NAOK = TRUE)$dns"))
...
extremaltform <- function(data, coord, cov.mod, loc.form, scale.form, shape.form,
    ...
    body(nplk) <- parse(text = paste("-.C(C_extremaltdsgnmat, as.integer(cov.mod.num),
...
as.double(DoF), dns = double(1), NAOK = TRUE)$dns"))
